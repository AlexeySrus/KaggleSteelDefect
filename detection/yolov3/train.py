...
def train(
    ...
    writer = SummaryWriter(logdir='tensorboard_logging/')
    init_seeds()
    ...
    # optimizer = optim.RMSprop(model.parameters(), lr=hyp['lr0'], weight_decay=hyp['weight_decay'])
        ...
        if not (opt.notest or (opt.nosave and epoch < 10)) or epoch == epochs - 1:
            with torch.no_grad():
                results, maps = test.test(cfg, data_cfg, batch_size=batch_size, img_size=img_size_test, model=model,
                                          conf_thres=0.1, writer=writer, epoch=epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--transfer', action='store_true', help='transfer lhttps://drive.google.com/drive/folders/1uxgUBemJVw9wZsdpboYbzUN4bcRhsuAIearning flag')
